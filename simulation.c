...
double timestep(const param_t params, const accel_area_t accel_area,
    ...
    speed_t* cells, int* obstacles)
{
    ...
    {
        jj = accel_area.idx;
	//#pragma omp for firstprivate(params,accel_area,cells,obstacles,w1,w2) private(ii)
    ...
    {
        ii = accel_area.idx;
	//#pragma omp for firstprivate(params,accel_area,cells,obstacles,w1,w2) private(jj)
...
void propagate(const param_t params, speed_t* cells, speed_t* tmp_cells, int* obstacles)
{
    int ii,jj;            /* generic counters */
    ...
    {
#pragma omp for private(ii,jj) firstprivate(cells, tmp_cells,obstacles) schedule(guided)
    for (ii = 0; ii < params.ny; ii++)
    {
        int y_n,y_s;  /* indices of neighbouring cells */
...
void rebound(const param_t params, speed_t* cells, speed_t* tmp_cells, int* obstacles)
{
    ...
    #pragma omp parallel
    {
    #pragma omp for firstprivate (cells,tmp_cells,obstacles) private(ii,jj) schedule(guided)
    for (ii = 0; ii < params.ny; ii++)
    {
        for (jj = 0; jj < params.nx; jj++)
        {
...
double collision(const param_t params, speed_t* cells, speed_t* tmp_cells, int* obstacles)
{
    ...
    double tot_u = 0;          /* accumulated magnitudes of velocity for each cell */

    /* loop over the cells in the grid
    ** NB the collision step is called after
    ** the propagate step and so values of interests
    ** are in the scratch-space grid */
    #pragma omp parallel proc_bind(spread)
    {
    #pragma omp for private(ii,jj,kk,u_x,u_y,u_sq,u,d_equ) firstprivate(cells,tmp_cells,obstacles) reduction(+:local_density) reduction(+:tot_cells) reduction(+:tot_u) schedule(guided)
    for (ii = 0; ii < params.ny; ii++)
    {
        for (jj = 0; jj < params.nx; jj++)
        {
		...
		for(kk = 1; kk <5; kk++){
		  d_equ[kk] = w1 * local_density * (1.0 + u[kk] / c_sq + (u[kk] * u[kk]) / (2.0 * c_sq * c_sq) - u_sq / (2.0 * c_sq));
		}
		for(kk = 5; kk <9; kk++){
		  d_equ[kk] = w2 * local_density * (1.0 + u[kk] / c_sq + (u[kk] * u[kk]) / (2.0 * c_sq * c_sq) - u_sq / (2.0 * c_sq));
		}	      
...
double av_velocity(const param_t params, speed_t* cells, int* obstacles)
{
    ...
    #pragma omp for firstprivate(cells, obstacles) reduction(+:tot_cells) reduction(+:tot_u) reduction(+:local_density) private(u_x, u_y, ii, jj, kk) schedule(guided) nowait
    for (ii = 0; ii < params.ny; ii++)
    {
        for (jj = 0; jj < params.nx; jj++)
        {
