    ...
    int** obstacles_ptr, double** av_vels_ptr)
{
    ...
    *av_vels_ptr = (double*) malloc(sizeof(double)*(params->max_iters));
    ...
    #pragma omp parallel
    {
    #pragma omp for firstprivate(params,cells_ptr,obstacles_ptr,w0,w1,w2) private(ii,jj) schedule(guided)
    ...
    #pragma omp for firstprivate(params,obstacles,obstacles_ptr,n_obstacles) private(ii,jj,kk) schedule(guided)
    for (ii = 0; ii < params->ny; ii++)
    {
        for (jj = 0; jj < params->nx; jj++)
        {
